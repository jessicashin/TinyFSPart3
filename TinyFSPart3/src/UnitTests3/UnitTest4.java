...
public class UnitTest4 {
	...
	public static void main(String[] args) {
		...
		System.out
				.println(TestName
						+ "Construct a record with the first four bytes equal to i, followed with 5 char attributes each with length 20.");
		...
		while (r1.getRID() != null) {
			...
			if (r2.getRID() != null) {
				...
				System.out.println("rid is null" + " " + cntr);
				r1.setRID(null);
			}

		}
		// System.out.println("after while loop "+cntr);

		System.out
				.println(TestName
						+ "Delete the odd numbered records using their first four bytes.");
		// Iterate the vector and delete the RIDs stored in it
		for (int i = 0; i < vect.size(); i++) {
			fsrv = crec.DeleteRecord(fh, vect.get(i));
			if (fsrv != FSReturnVals.Success) {
				System.out
						.println("Unit test 4 result: failed to delete the record!");
				return;
			}
		}
		...
		fsrv = cfs.CloseFile(fh);
		if (cntr != NumRecs) {
			System.out.println("Unit test 4 result: fail!" + cntr + " "
					+ NumRecs);
			// return;
		}
		...
		System.out
				.println(TestName
						+ "Scan the file and verify there are only even numbered records using their first four bytes.");
		ofd = cfs.OpenFile("/" + dir1 + "/emp", fh);
		r1 = new TinyRec();
		...
		while (r1.getRID() != null) {
			...
			if (r2.getRID() != null) {
				...
				if (value % 2 != 0) {
					System.out
							.println("Unit test 4 result: fail!  Found an odd numbered record with value "
									+ value + ".");
					return;
				}
				r1 = r2;
